 ...
 * setColor method.
 * 
 * @author David J. Barnes, Michael Kolling and Abraão Dantas
 * @version 2003.12.22
 */
public class SimulatorView extends JFrame
{
    ...
    public SimulatorView(int height, int width)
    {
        ...
        colors = new HashMap<Class<? extends Actor>,Color>();
     ...
     */
    public void setColor(Class<? extends Actor> fishClass, Color color)
    {
    ...
    public void showStatus(int step, Ocean ocean)
    {
        ...
        for(int row = 0; row < ocean.getHeight(); row++) {
            for(int col = 0; col < ocean.getWidth(); col++) {
            	if(ocean.getFishAt(row, col) instanceof Fish) {
            		actor = ocean.getFishAt(row, col);
            	}else if(ocean.getSeaweedAt(row, col) instanceof Seaweed) {
            		actor = ocean.getSeaweedAt(row, col);
            	}
                if(actor != null) {
                	if(actor instanceof Fish) {
                		stats.incrementCount(((Fish) actor).getClass());
                        oceanView.drawMark(col, row, getColor(actor.getClass()));
                	}else {
                		//stats.incrementCount(((Seaweed)actor).getClass());
                        oceanView.drawMark(col, row, getColor(actor.getClass()));
                	}
